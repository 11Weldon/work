...
from src.dal.schemas.channel import ChannelORM, ChannelListORM
from src.dal.schemas.product import GroupProductORM
...
async def add_channel(session: AsyncSession, channel_data):
    try:
        new_channel = ChannelORM(**channel_data.dict())
...
async def get_channel(session: AsyncSession) -> list[ChannelORM]:
...
async def set_channel_live_urls(session: AsyncSession, channel_live_urls):
    try:
        channel = await session.execute(
            update(ChannelORM).
            where(channel_live_urls.channel_id == ChannelORM.channel_id).
...
async def get_group_product_with_channels(session: AsyncSession):
    stmt = select(GroupProductORM).options(selectinload(GroupProductORM.channels))
...
async def create_services_list(session: AsyncSession, list_data):
    try:
        new_list = ChannelListORM(**list_data.dict())
