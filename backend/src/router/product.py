...
from backend.src.queries import product
from backend.src.schemas.product import GroupProductSchema, FeatureProductSchema, ProductSchema
...
@product_router.post("/op_facade/prodMgmt/CreateProduct")
async def create_product(product_data: ProductSchema,
                         session: AsyncSession = Depends(get_db)
                         ):
    try:
        new_product = await product.add_feature_product(session, product_data)
        return new_product
    except Exception as ex:
        raise HTTPException(status_code=500, detail=str(ex))
